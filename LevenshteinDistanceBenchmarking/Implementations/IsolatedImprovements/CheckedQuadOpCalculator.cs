...
{
	class CheckedQuadOpCalculator : ILevenshteinDistanceSpanCalculator
	{
		public int CalculateDistance(ReadOnlySpan<char> source, ReadOnlySpan<char> target)
		{
			...
			{
				var jumpSize = 4;
				var doMultiOp = target.Length > 3;
				for (var j = 1; j <= target.Length; j += jumpSize)
				{
					if (doMultiOp)
					{
					...
					else
					{
						...
						var result = Math.Min(Math.Min(insert, delete), edit);
						costMatrix[i][j] = result;
					}
				}
			}
